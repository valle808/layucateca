        ...
        const inspectRes = await send('Runtime.evaluate', {
            expression: `
                (function() {
                    ...
                    const envInputs = inputs.filter(i => {
                    ...
                    const envButtons = buttons.filter(b => {
                        const text = b.innerText.trim().toLowerCase();
                        return text.includes('add') || text.includes('save') || text.includes('create') || text.includes('submit');
                    });
                    
                    return {
                        envInputs: envInputs.map(i => ({
                            id: i.id,
                            name: i.name,
