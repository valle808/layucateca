        ...
        const inspectRes = await send('Runtime.evaluate', {
            expression: `
                (function() {
                    ...
                    const dnsInputs = inputs.filter(i => {
                        const html = i.outerHTML.toLowerCase();
                        return html.includes('name') || html.includes('value') || html.includes('ttl') || html.includes('priority') || html.includes('comment') || html.includes('dns') || html.includes('subdomain');
                    });
                    
                    const addButtons = buttons.filter(b => {
                        return b.innerText.trim().toLowerCase() === 'add';
                    });
                    
                    return {
                        dnsInputs: dnsInputs.map(i => ({
                            id: i.id,
                            name: i.name,
