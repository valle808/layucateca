...
async function main() {
    ...
    ws.on('open', async () => {
        ...
        const evalRes = await send('Runtime.evaluate', {
            expression: `
                (function() {
                    const btns = Array.from(document.querySelectorAll('*')).filter(el => el.innerText && el.innerText.trim().toUpperCase() === 'DESTROY');
                    return btns.map(b => ({
                        tagName: b.tagName,
        ...
        console.log('DESTROY Buttons found:', JSON.stringify(evalRes.result.value, null, 2));
