...
export default function OpinionRoomPage() {
  ...
  return (
    <main className="min-h-screen pt-24 pb-8 px-4 md:px-8 max-w-7xl mx-auto flex flex-col h-screen font-sans relative">
      
      {/* Modern Glassmorphism Background Elements */}
      <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden bg-[#fafafa] dark:bg-[#09090b] transition-colors">
        <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-gradient-to-br from-[#ff5500]/20 to-[#ff0080]/20 rounded-full blur-[140px] mix-blend-multiply dark:mix-blend-screen opacity-70"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-gradient-to-tl from-blue-500/20 to-emerald-500/20 rounded-full blur-[120px] mix-blend-multiply dark:mix-blend-screen opacity-70"></div>
      </div>

      {/* Upper header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 shrink-0 gap-4">
        <div>
          <h1 className="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-slate-400 tracking-tight flex items-center gap-3">
            <MessageSquare className="w-10 h-10 text-[#ff5500]" />
            <span>{t("Sala de Opinión", "Opinion Room", "Sala de Opinión")}</span>
          </h1>
          <p className="text-sm md:text-base text-slate-500 dark:text-white/60 mt-2 font-medium max-w-xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-slate-200/50 dark:border-white/10 rounded-2xl py-2 px-4 shadow-sm hover:shadow-md hover:bg-white/80 dark:hover:bg-white/10 transition-all cursor-pointer group"
          >
            ...
            <div className="text-left">
              <span className="text-[10px] text-slate-500 dark:text-white/50 uppercase font-bold tracking-wider block">Tu Nombre</span>
              <span className="text-sm font-black text-slate-900 dark:text-white truncate max-w-[100px] block">{getActiveDisplayName()}</span>
            </div>
          </button>

          {user && (
            <div className="flex items-center gap-3 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-slate-200/50 dark:border-white/10 rounded-2xl py-2 px-4 shadow-sm">
              <div className="p-1.5 bg-yellow-500/10 rounded-lg border border-yellow-500/20">
                <Trophy className="w-5 h-5 text-yellow-500 dark:text-yellow-400" />
              </div>
              <div className="text-left">
                <span className="text-[10px] text-slate-500 dark:text-white/50 uppercase font-bold tracking-wider block">Reputación</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 grow overflow-hidden">
        ...
        {/* Rooms Sidebar */}
        <div className="md:col-span-4 flex flex-col bg-white/40 dark:bg-[#111116]/40 backdrop-blur-2xl border border-white/50 dark:border-white/5 rounded-3xl overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
          <div className="flex justify-between items-center p-5 border-b border-white/40 dark:border-white/5 bg-white/30 dark:bg-black/20 shrink-0">
          ...
          <div className="space-y-2 overflow-y-auto grow p-4 custom-scrollbar">
            {loadingRooms ? (
        ...
        {/* Message Stream */}
        <div className="md:col-span-8 flex flex-col bg-white/60 dark:bg-[#111116]/60 backdrop-blur-2xl border border-white/50 dark:border-white/5 rounded-3xl overflow-hidden shadow-[0_12px_40px_rgb(0,0,0,0.06)] dark:shadow-[0_12px_40px_rgb(0,0,0,0.3)] h-full relative">
          
          {/* Room Title */}
          <div className="bg-white/40 dark:bg-black/20 backdrop-blur-xl border-b border-white/50 dark:border-white/5 p-5 flex justify-between items-center shrink-0 z-10">
            ...
            <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 px-3 py-1.5 rounded-full shadow-sm">
              ...
              <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">En Vivo</span>
            </div>
          </div>

          {/* Messages list */}
          <div className="grow p-6 overflow-y-auto space-y-6 min-h-0 custom-scrollbar relative">
          ...
          <form 
            ...
            className="p-4 bg-white/80 dark:bg-[#111116]/80 backdrop-blur-2xl border-t border-slate-200 dark:border-white/10 flex gap-3 shrink-0 z-10"
          >
            {/* Hidden File Input */}
            <input 
              type="file" 
            ...
            <button
              ...
              className="p-3.5 rounded-2xl bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-white/60 hover:bg-slate-200 dark:hover:bg-white/10 hover:text-[#ff5500] dark:hover:text-[#ff5500] transition-colors"
            ...
            <button
              ...
              className="p-3.5 rounded-2xl bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-white/60 hover:bg-slate-200 dark:hover:bg-white/10 hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors"
            ...
            <input
              type="text"
              placeholder="Escribe un mensaje asombroso..."
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              disabled={!!fetchError}
              className="input text-[15px] grow py-3.5 px-6 bg-white dark:bg-black/20 border border-slate-200 dark:border-white/10 focus:border-[#ff5500]/50 focus:ring-2 focus:ring-[#ff5500]/20 rounded-2xl placeholder-slate-400 dark:placeholder-white/30 shadow-inner"
            />
            
            <button
              type="submit"
              disabled={!!fetchError || (!inputText.trim() && !attachedImage && !attachedLocation)}
              className="btn-primary px-6 rounded-2xl flex items-center justify-center shrink-0 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-orange-500/30 hover:shadow-xl hover:shadow-orange-500/40 hover:scale-[1.02] active:scale-95 transition-all"
            >
